...
void TIMER0_Handler(void)
{
	...
	reg_val = T0->CON; // Get Status
	
	// GRA Match Flag
	if ((T0->CON & TnCON_TIF0) == TnCON_TIF0) {
		reg_val  &= ~(TnCON_IFLAG_MASK);
		reg_val |=  TnCON_TIF0;
		T0->CON = reg_val; // Clear TIF0 (GRA Match) Flag Status
	}
	
	// GRB Match Flag
	if ((T0->CON & TnCON_TIF1) == TnCON_TIF1) {
		reg_val &= ~(TnCON_IFLAG_MASK);
		reg_val |=  TnCON_TIF1;
		T0->CON = reg_val; // Clear TIF1 (GRB Match) Flag Status
	}	
	// OVF Flag
	if ((T0->CON & TnCON_TOVF) == TnCON_TOVF) {
		reg_val &= ~(TnCON_IFLAG_MASK);
		reg_val |=  TnCON_TOVF;
		T0->CON = reg_val; // Clear OVF Flag Status
	} 
	g_TIMER0_count++; 
}
