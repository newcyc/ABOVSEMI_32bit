/***************************************************************************//**
* @file     main.c
* @brief    An example of UART using interrupt(DMA) mode on A31G22x
...
#define USED_DMAC					(DMAC1) // DMAC0 ~ DMAC7
...
"\t - Using UART interrupt(DMA) mode\r\n"
"************************************************\r\n";

static Bool TxDoneFlag;

static uint8_t TestData[12] = {
...
void UART1_IRQHandler(void)
{
	...
	if (Reg32_IIR == UART_IIR_IID_THRE) {
		TxDoneFlag = TRUE;
	}
}
...
static void Main_MainLoop(void)
{
	DMAC_CFG_Type DMAC_Config;
	UART_CFG_Type UART_Config;

	Main_PrintMenu();

	// DMAC setting 
	DMAC_Config.Peripheral = DMAC_PERIPHERAL_UART1_TX;
	DMAC_Config.Width = DMAC_WIDTH_BYTE;
	DMAC_Config.Direction = DMAC_DIRECTION_TX;
	DMAC_Config.PeripheralAddress = (uint32_t)(&(UART1->THR));
	DMAC_Init(USED_DMAC, &DMAC_Config);
	...
	UART_ConfigureInterrupt(UART1, UART_INTERRUPT_THREIE, ENABLE);
	...
	while (1) {
		DMAC_SetTransferCount(USED_DMAC, sizeof(TestData));
		DMAC_SetMemoryLowAddress(USED_DMAC, (uint16_t)((uint32_t)(&(TestData[0])) & 0x0000FFFF));
		TxDoneFlag = FALSE;
		DMAC_Start(USED_DMAC);
