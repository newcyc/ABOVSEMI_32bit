/***************************************************************************//**
* @file     main.c
* @brief    An example of UART using Tx interrupt mode on A31G22x
...
"\t - Using UART1 interrupt(Tx) mode\r\n"
"************************************************\r\n";

static Bool BusyFlag;

static uint8_t *pTx_Buffer;
static uint32_t Tx_Index;
static uint32_t Tx_BufferLength;


/*******************************************************************************
* Private Function Prototype
*******************************************************************************/
static void Main_CheckTxBuffer(UART_Type *pUARTx, uint8_t *pTxBuffer, uint32_t BufferLength);
...
void UART1_IRQHandler(void)
{
	...
	if (Reg32_IIR == UART_IIR_IID_THRE) {
		if (++Tx_Index < Tx_BufferLength) {
			UART_SendByte(UART1, pTx_Buffer[Tx_Index]);
		} else {
			BusyFlag = FALSE;
		}
	}
...
/*******************************************************************************
* @brief      Check Tx Buffer. Update new text if buffer is empty
...
static void Main_CheckTxBuffer(UART_Type *pUARTx, uint8_t *pTxBuffer, uint32_t BufferLength)
{
	if (BusyFlag == FALSE) {
		BusyFlag = TRUE;
		Tx_Index = 0;
		pTx_Buffer = pTxBuffer;
		Tx_BufferLength = BufferLength;
		UART_SendByte(pUARTx, pTx_Buffer[Tx_Index]);
	}
}
...
static void Main_MainLoop(void)
{
	...
	uint8_t Data[14] = {'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', '\n', '\r'};
	...
	while (1) {
		Main_CheckTxBuffer(UART1, Data, sizeof(Data));
	}
}
