/***************************************************************************//**
* @file     main.c
* @brief    An example of UART using polling mode on A31G22x
...
static void Main_MainLoop(void)
{
	...
	UART_Send(UART1, "Polling test\r\n", 15, BLOCKING);
	while (1) {
		UART_Receive(UART1, &Data, 1, BLOCKING);
		UART_Send(UART1, &Data, 1, BLOCKING);

		if ((Data == ASCII_CODE_LF) || (Data == ASCII_CODE_CR)) {
			if (((Data == ASCII_CODE_LF) && (PreData != ASCII_CODE_CR))
				|| ((Data == ASCII_CODE_CR) && (PreData != ASCII_CODE_LF))) {
				UART_Send(UART1, "\r\n", 2, BLOCKING);
				PreData = Data;
			} else {
				PreData = Data;
