/***************************************************************************//**
* @file     main.c
* @brief    An example of UART using polling(DMA) mode on A31G22x
...
#define USED_DMAC					(DMAC2) // DMAC0 ~ DMAC7
...
static void Main_MainLoop(void)
{
	...
	while (1) {
		...
		while (DMAC_GetStatus(USED_DMAC) == RESET);
		DMAC_Stop(USED_DMAC);
	}
}
