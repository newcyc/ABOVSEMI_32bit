...
void dataFlash(void);
void selfPgmErase(void);
void flashSize(void);

int FLASH_TEST_Verify (uint8_t * src, uint8_t * dest, int count);
int FLASH_TEST_disp_memory (uint32_t addr);
...
void mainloop(void)
{
	...
	for (;;)
	{
		...
		if (InFlag)
		{
			...
			{
				cputs(">>> Data Flash Test\r\n");
				dataFlash();				
			}	
			else if (!strncmp(InData, "2", 1))
			{
				cputs(">>> Code Flash : Self PGM & Self ERASE Test\r\n");
				selfPgmErase();				
			}
			else if (!strncmp(InData, "3", 1))
			{
				cputs(">>> Code Flash Size Check\r\n");
				flashSize();				
			}
...
void Disp_MainMenu(void)
{
	...
	cprintf("1. Data Flash Test\r\n");
	cprintf("2. Code Flash : Self PGM & Self ERASE Test\r\n");	
	cprintf("3. Code Flash Size Check \r\n");	
	cprintf("========================================\r\n"); 
}
...
uint8_t	g_flash_buf[1024]; 
void dataFlash(void)
{
	uint32_t		reg_val;
	int 			extended, protect;

	int			error; 
	uint32_t		addr = 0x0F000000; 
	uint8_t		data = 0xAB;
	int i;
	...
	extended = 1;
//	extended = 0;	
	// extended : Enable Self PGM/ERASE/Boot Block Protection 
	if (extended == 1)
	{
...
//	protect = 0;	
	if (protect == 1)
	{
		FLASH_DisableProtection (FMC, FMPROTECT_DPx_ALL);  
	}
	else 
	{
		FLASH_EnableProtection (FMC, FMPROTECT_DPx_ALL); 
	}
	...
	cprintf("extended=%d\r\n", extended); 
	cprintf("write enable=%d\r\n", protect); 
	cprintf("FMPROT(0x4000_011C)=0x%08X\r\n", reg_val); 
	cprintf("FMTEST(0x4000_0114)=0x%08X\r\n", extended); 
	cprintf("Initialized for Data Flash Test...\r\n"); 
	cprintf("\r\n");
	...
	for(i = 0 ; i < 0x3200; i++)
	{
		error = FLASH_ProgramByte (FMC, addr+i, (uint8_t) data); 
		cprintf("Program 0x%X @ 0x%08X : 0x%08X done...\r\n", data, addr+i, (*(volatile unsigned long *)addr+i)); 
		if (error)
		{
			cprintf("PGM fail\r\n"); 
		}
	...
	cprintf("PGM done...\r\n");
	FLASH_TEST_disp_memory(addr);
	
	
	///////////////////////////////////////////////////////////////////////////
	// Chip Erase
	///////////////////////////////////////////////////////////////////////////	
	error = FLASH_EraseChip (FMC, addr); 
	cprintf("Chip Erase done...\r\n"); 
	if (!error)
	{
		cprintf("success\r\n"); 
	}
	...
	///////////////////////////////////////////////////////////////////////////
	// Program Byte : 
	// Notice!) Erase must be performed before programming flash memory.
	///////////////////////////////////////////////////////////////////////////		
	for(i = 0 ; i < 0x80; i+=2)
	{
		error = FLASH_ProgramByte (FMC, addr+i, (uint8_t) data); 
//		cprintf("Program 0x%X @ 0x%08X done...\r\n", data, addr+i); 
		if (error)
		{
			cprintf("PGM fail\r\n"); 
		}
	...
	// Sector Erase
	///////////////////////////////////////////////////////////////////////////	
	error = FLASH_EraseSector (FMC, addr); 
	cprintf("Erase 1 KB sector @ 0x%08X\r\n", addr); 
...
void selfPgmErase(void)
{
	uint32_t 		protect, extended, reg_val; 

	int			error, i;
	uint32_t		addr = 0x00040000; 
	...
	// Notice!) Erase must be performed before programming flash memory.
	///////////////////////////////////////////////////////////////////////////
	cprintf("SELF-PROGRAM...\r\n\r\n"); 		
	for(i = 0 ; i < 0x80; i+=4)
	{
		error = FLASH_Self_ProgramWORD (FMC, addr+i, data); 
	...
	cprintf("self-pgm sector : addr=0x%08X\r\n", addr); 
	FLASH_TEST_disp_memory(addr);	
	///////////////////////////////////////////////////////////////////////////
	// Self Sector Erase
	///////////////////////////////////////////////////////////////////////////
	cprintf("SELF-SECTOR ERASE..\r\n\r\n"); 			
	error = FLASH_Self_EraseSector (FMC, addr); 
	...
	cprintf("self-erase sector : addr=0x%08X\r\n", addr); 
	FLASH_TEST_disp_memory(addr);	
...
void flashSize(void)
{
	...
	// Flash HWID and Flash Size are Fixed values (Read-only)
	cprintf("Flash HWID : ");
	hwid = FMC->HWID;
	if(hwid == 0x33527000)
	{
		cprintf("A33G527 (0x%08X)\r\n", hwid);
	}
	else if(hwid == 0x33526000)
	{
		cprintf("A33G526 (0x%08X)\r\n", hwid);
	}
	
	cprintf("Code Flash Size : ");
	size = FMC->SIZE;	
	if(size == 0x5A)
	{
		cprintf("384KB\r\n");
...
//	addr = 0x0F000000; 
	...
	for (offset=0; offset<0x80; offset+=0x10)
	{
		...
		temp[0] = *((volatile unsigned long *) (addr+offset+0x00)); 
		temp[1] = *((volatile unsigned long *) (addr+offset+0x04)); 
		temp[2] = *((volatile unsigned long *) (addr+offset+0x08)); 
		temp[3] = *((volatile unsigned long *) (addr+offset+0x0C)); 

		cprintf("0x%08X : 0x%08X, 0x%08X, 0x%08X, 0x%08X\r\n", (addr+offset), temp[0], temp[1], temp[2], temp[3]); 
	}
...
int FLASH_TEST_Verify (uint8_t * src, uint8_t * dest, int count)
{
	...
	for (i=0; i<count; i++)
	{
		...
		{
			error = 1; 
			cprintf("sdata=0x%02X, ddata=0x%02X\r\n", sdata, ddata); 
			cprintf("src=0x%08X, dest=0x%08X\r\n", (uint32_t) src, (uint32_t) dest); 
