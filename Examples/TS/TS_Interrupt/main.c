/***************************************************************************//**
* @file     main.c
* @brief    An example of TS(Temperature Sensor) using interrupt mode on A31G22x
...
"************************************************\r\n"
" TS(Temperature Sensor) demo\r\n"
"\t - MCU: A31G22x\r\n"
"\t - Core: ARM Cortex-M0+\r\n"
"\t - Communicate via: USART10 - 38400 bps\r\n"
"\t - Using TS interrupt mode\r\n"
"************************************************\r\n";

static Bool TS_DoneFlag;
static Bool TS_PortFlag;
...
static void Main_MainLoop(void)
{
	...
	// Clock setting
	SCU_SetTSClock(ENABLE, SCU_TS_REFERENCE_MCLK, SCU_TS_SESNING_LSI_TS);

	// TS peripheral setting
	TS_Config.MatchInterrupt = ENABLE;
	TS_Config.ReferencePeriod = 32000 * 2; // 32000 * 2 / 32Mhz = 2ms
	TS_Init(&TS_Config);

	NVIC_SetPriority(TEMP_SENSOR_IRQn, 3);
	NVIC_EnableIRQ(TEMP_SENSOR_IRQn);
	__enable_irq();
	...
	while(1) {
		if (TS_DoneFlag == TRUE) {
			TS_DoneFlag = FALSE;
			TS_Stop();
			Count = TS_GetSensingCount();
			_DBG("Count = "); _DBD32(Count); _DBG_(" ");
