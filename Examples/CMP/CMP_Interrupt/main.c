/***************************************************************************//**
* @file     main.c
* @brief    An example of comparator using interrupt mode on A31G22x
...
"\t - Using CMP interrupt mode\r\n"
"************************************************\r\n";
#if (USED_CMP_N == 0)
static const CMP_NUMBER_Type CMP_NUMBER = CMP_0;
static const uint32_t TEST_PIN_NUMBER = 2; // PE2
static const uint32_t SOURCE_PIN_NUMBER = 2; // PA2
#else
static const CMP_NUMBER_Type CMP_NUMBER = CMP_1;
static const uint32_t TEST_PIN_NUMBER = 3; // PE3
static const uint32_t SOURCE_PIN_NUMBER = 5; // PA5
#endif
...
/*******************************************************************************
* @brief      Interrupt Handler of Comparator and CRC
* @param      None
* @return     None
*******************************************************************************/
void CMP_CRC_IRQHandler(void)
{
	if (CMP_GetInterruptStatus(CMP_NUMBER) == SET) {
		if (TestPin_Output == 1) { // toggle test pin
			GPIO_ClearValue(PE, _BIT(TEST_PIN_NUMBER));
		} else {
			GPIO_SetValue(PE, _BIT(TEST_PIN_NUMBER));
		}
		TestPin_Output ^= 1;
		Main_Delay();

		CMP_ClearInterruptStatus(CMP_NUMBER);
	}
}
...
static void Main_MainLoop(void)
{
	...
	// Test pin setting
	PCU_ConfigureDirection(PE, TEST_PIN_NUMBER, PCU_MODE_PUSH_PULL);
	PCU_ConfigurePullupdown(PE, TEST_PIN_NUMBER, PCU_PUPD_DISABLE);	
	GPIO_ClearValue(PE, _BIT(TEST_PIN_NUMBER));
	TestPin_Output = 0;

	// Source pin setting
	// Source : CMP0 -> CP0(PA2), CMP1 -> CP1A(PA5)
	PCU_ConfigureFunction(PA, SOURCE_PIN_NUMBER, PCU_ALT_FUNCTION_3);
	PCU_ConfigureDirection(PA, SOURCE_PIN_NUMBER, PCU_MODE_INPUT);
	PCU_ConfigurePullupdown(PA, SOURCE_PIN_NUMBER, PCU_PUPD_DISABLE);

	// Initialize comparator
	CMP_Init();

	// Configure comparator
	CMP_ConfigureHysterisis(CMP_NUMBER, CMP_HYSSEL_5_MILLIVOLTS, ENABLE);

	CMP_ConfigureInput(CMP_NUMBER, CMP_SOURCE_0, CMP_REFERENCE_1);

	CMP_ConfigureDBNCCounter(24);
	CMP_ConfigureDBNCNumber(CMP_NUMBER, 0);

	CMP_ConfigureInterrupt(CMP_NUMBER, CMP_TRIGGER_POL_NORMAL, CMP_INTERRUPT_POL_HIGH, CMP_INTERRUPT_MODE_LEVEL);
	CMP_EnableInterrupt(CMP_NUMBER, ENABLE);

	// Enable interrupt
	NVIC_SetPriority(CMP_CRC_IRQn, 0);
	NVIC_EnableIRQ(CMP_CRC_IRQn);

	__enable_irq();

	CMP_Start(CMP_NUMBER);
