/***************************************************************************//**
* @file     main.c
* @brief    An example of 32-bit timer2n using capture mode on A31G22x
...
* Private Typedef
*******************************************************************************/
// Timer2n capture demo configuration structure definition
typedef struct {
	// base address
...
"\t - Using timer2n capture mode\r\n"
"************************************************\r\n";

static const TIMER2n_CAPTURE_Type TIMER2n_CAPTURE_CFG[TIMER2n_IP_INDEX_MAX] = {
	{TIMER20, TIMER20_IRQn, PC, 0, PCU_ALT_FUNCTION_2},
	{TIMER21, TIMER21_IRQn, PC, 1, PCU_ALT_FUNCTION_2}
};

static Bool Timer2nFlag;
...
*******************************************************************************/
static void Main_Timer2nIRQHandler(TIMER2n_Type *pTIMER2x);
...
{
	Main_Timer2nIRQHandler(TIMER20);
}
...
{
	Main_Timer2nIRQHandler(TIMER21);
}
...
/*******************************************************************************
* @brief      Timer2n Interrupt Handler
* @param      None
* @return     None
*******************************************************************************/
static void Main_Timer2nIRQHandler(TIMER2n_Type *pTIMER2x)
{
	if (TIMER2n_GetStatus(pTIMER2x) & TIMER2n_STATUS_CAPTURE_INT) {
		if (Timer2nFlag == FALSE) {
			Timer2nFlag = TRUE;
		}

		TIMER2n_ClearStatus(pTIMER2x, TIMER2n_STATUS_CAPTURE_INT);
	}
}
...
static void Main_MainLoop(void)
{
	volatile uint32_t i;
	uint32_t Count;
	const TIMER2n_CAPTURE_Type * pConfig;

	pConfig = &(TIMER2n_CAPTURE_CFG[TARGET_TIMER]);
	...
	// Timer2n and clock source setting
	SCU_SetTimer20Clk(T20CLK_PCLK);

	TIMER2n_Config.ExtClock = DISABLE; // PCLK
	TIMER2n_Config.Mode = TIMER2n_MODE_CAPTURE;
	TIMER2n_Config.CaptureSignal = TIMER2n_CAPTURE_SIGNAL_EXTERNAL;
	TIMER2n_Config.CapturePolarity = TIMER2n_CAPTURE_POLARITY_RISING;
	TIMER2n_Config.MatchInterrupt = DISABLE;
	TIMER2n_Config.CaptureInterrupt = ENABLE;

	TIMER2n_Config.AData = 0xFFFF;

	TIMER2n_Config.PrescalerData = 32 - 1; // 32MHz / 32 = 1MHz -> 1us

	TIMER2n_Init(pConfig->pTIMER2x, &TIMER2n_Config);

	NVIC_SetPriority(pConfig->TIMER2x_IRQ, 3);

	NVIC_EnableIRQ(pConfig->TIMER2x_IRQ);
	...
	Timer2nFlag = FALSE;
	TIMER2n_ClearCounter(pConfig->pTIMER2x);
	...
	while (1) {
		if (Timer2nFlag == TRUE) {
			_DBG_("capture value!");
			Count = TIMER2n_GetCaptureData(pConfig->pTIMER2x);
			TIMER2n_ClearCounter(pConfig->pTIMER2x);
