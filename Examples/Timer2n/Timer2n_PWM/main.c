/***************************************************************************//**
* @file     main.c
* @brief    An example of 32-bit timer2n using PWM mode on A31G22x
...
#define TIMER2n_CLOCK				(USED_CLOCK_PCLK)
...
* Private Typedef
*******************************************************************************/
// Timer2n PWM demo configuration structure definition
typedef struct {
	// base address
...
"\t - Using timer2n PWM mode\r\n"
"************************************************\r\n";

static const TIMER2n_PWM_Type TIMER2n_PWM_CFG[TIMER2n_IP_INDEX_MAX] = {
...
static void Main_Timer2nIRQHandler(TIMER2n_Type *pTIMER2x)
{
	if (TIMER2n_GetStatus(pTIMER2x) & TIMER2n_STATUS_MATCH_INT) {
		Timer2nCount++;
		if (Timer2nCount >= 200) {
			Timer2nCount = 0;
		...
		TIMER2n_ClearStatus(pTIMER2x, TIMER2n_STATUS_MATCH_INT);
	}
}
...
static void Main_MainLoop(void)
{
	const TIMER2n_PWM_Type * pConfig;

	Timer2nFlag = FALSE;
	Timer2nCount = 0;

	pConfig = &(TIMER2n_PWM_CFG[TARGET_TIMER]);
	...
	TIMER2n_Config.AData = 2000; // 2ms

	TIMER2n_Config.BData = 500; // 0.5ms
