...
void spiTest(void)
{
	...
	uint32_t		master_send[8] = {0x12, 0x34, 0x56, 0x78, 0xAB, 0xCD, 0xEF, 0xFF};
	uint32_t		master_receive[8];
	
	uint32_t		slave_send[8] = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x11, 0x22};
	uint32_t		slave_receive[8]; 
	
	uint32_t		count, m_read_count, s_read_count; 
	int			i, result; 
...
//	spi_config.baudrate = 49;
	...
	for (i=0; i<count; i++)
		spi_s_buffer->u32TxBuffer[i] = slave_send[i]; 
	...
	// read 
	//-----------------------------------------------------------------------------
	m_read_count = SPI_Read_Data(0, SPI_MASTER, master_receive, count); 
	...
	for (i=0; i<count; i++)
	{
		cprintf("0x%02X, ", slave_send[i]); 
	}

	cprintf("\r\n"); 
    
	cprintf("master receive: "); 
	for (i=0; i<m_read_count; i++)
	{
		cprintf("0x%02X, ", master_receive[i]); 
	}
