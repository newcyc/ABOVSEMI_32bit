...
//void spiTest(void);
void AT45DBXX_Application(void);
\
...
void Disp_MainMenu(void)
{
	...
	cprintf("1. SPI0(Master) <-> EEPROM Send and Receive\r\n");
	cprintf("========================================\r\n"); 
}
...
void AT45DBXX_Application(void) {
	uint8_t num,i;
	uint8_t ID[4];
	uint8_t s_buffer[256],r_buffer[256];
	
	cprintf("\r\n****************************************************************\r\n");

	AT45DBXX_Init();
	cprintf("AT45DBXX had been Init!\r\n");    
	AT45DBXX_Read_ID(ID);
	cprintf("AT45DBXX ID is");
	for(num = 0; num < 4; num++) {
        cprintf("0x%02X ", ID[num]);
	}
	cprintf("\r\n****************************************************************\r\n");
	cprintf("\r\n");
	
	cprintf("\r\n****************************************************************\r\n");
	cprintf("Buffer Write !! \r\n");
	write_buffer(0x02, 10);
	write_buffer(0x04, 9);
	
    
	cprintf("Buffer Read !! \r\n");
	cprintf(" SRAM Buffer : 0x02 : ");
	cprintf("0x%02X ", read_buffer(0x02));
	cprintf("\r\n SRAM Buffer : 0x04 : ");
	cprintf("0x%02X ", read_buffer(0x04));
	cprintf("\r\n****************************************************************\r\n");
	cprintf("\r\n");
	...
	cprintf("\r\n****************************************************************\r\n");
	cprintf("BUFFER 1 TO MAIN MEMORY PAGE PROGRAM \r\n");
	cprintf("PAGE SIZE : 264 byte \r\n");
	cprintf("PAGE 10, PROGRAM VALUE 0x0 - 0xFE\r\n");
	
	for(i=0;i<255;i++) 	{
		s_buffer[i] = i;
	}
	flash_write_page(10,s_buffer,0);
	
	cprintf("\r\nREAD BUFFER 1 TO MAIN MEMORY PAGE ");	

	flash_read_page(10,r_buffer);
//	for(i=0;i<255;i++) 	{
//		if(i%8==0) cprintf("\r\n");
//		cprintf("0x%02X \r\n", (r_buffer[i]));
//	}
	for(i=0;i<255;i++) {
		if(i%8==0) cprintf("\r\n");
		cprintf("0x%02X ", r_buffer[i]);
		cprintf(" ");
	}    
cprintf("\r\n****************************************************************\r\n");	
cprintf("\r\n");	
	
cprintf("FLASH ERASE PAGE !! \r\n");
...
cprintf("\r\n");	
	...
	while (1) {
	/*====LED-ON=======*/
    GPIO_OutputHigh(PB, 0);
    GPIO_OutputHigh(PB, 1);
    GPIO_OutputHigh(PB, 2);
    GPIO_OutputHigh(PB, 3);
