...
void spiTest(void)
{
    ...
    SPS_config.BaudRate = 74; // PCLK / (74+1) 

    // SPI0 EANBLE
    SPI_Init(SPI0, &SPS_config);
    SPI_Cmd(SPI0, ENABLE);  	
    ...
    SPM_config.BaudRate = 74; // PCLK / (74+1) 
    
    // SPI1 ENABLE
    SPI_Init(SPI1, &SPM_config);
    SPI_Cmd(SPI1, ENABLE);  

    cprintf(" ");
    
    i=0x30;
    cprintf("init1 Tx=0x%02X \r\n", i);	
    while(1) {
        
        // CHECK TX BUFFER EMPTY
        while((SPI_GetStatus(SPI1) & SPI_STAT_TXBUF_EMPTY) != SPI_STAT_TXBUF_EMPTY);  
        ...
        rxdata = SPI_ReceiveData(SPI0);
        cprintf(" Tx= 0x%X, Rx=0x%02X\r\n", i, rxdata);
