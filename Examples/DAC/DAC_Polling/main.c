/***************************************************************************//**
* @file     main.c
* @brief    An example of DAC using polling mode on A31G22x
...
static void Main_MainLoop(void)
{
	...
	DAC_Config.OutBuffer = DISABLE; // Bypass Buffer
	DAC_Config.ReloadSignal = DAC_RELOAD_SIGNAL_ALWAYS;
	DAC_Init(DAC, &DAC_Config);

	// Initialize DAC data (No gain, No Offset)
	DAC_Data = 0x00;
	DAC_SetData(DAC, (uint16_t)(DAC_Data << 4));

	DAC_Start(DAC);

	// Make saw tooth waveform
	while (1) {
		DAC_Data++;
		DAC_Data &= 0x0FFFUL;
		DAC_SetData(DAC, (uint16_t)(DAC_Data << 4));
