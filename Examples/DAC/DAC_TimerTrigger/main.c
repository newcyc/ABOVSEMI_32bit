/***************************************************************************//**
* @file     main.c
* @brief    An example of DAC using timer trigger mode on A31G22x
...
#define TIMER10_INDEX				(0)
#define TIMER11_INDEX				(1)

#define TIMER_SOURCE				(TIMER10_INDEX) // TIMER10_INDEX ~ TIMER11_INDEX
...
"\t - Using DAC timer trigger mode\r\n"
"************************************************\r\n";
#if (TIMER_SOURCE == TIMER10_INDEX)
static TIMER1n_Type * const pTIMER1x = TIMER10;
static const IRQn_Type TIMER1x_IRQ = TIMER10_IRQn;
static const DAC_RELOAD_SIGNAL_Type RELOAD_SIGNAL = DAC_RELOAD_SIGNAL_TIMER10;
#else
static TIMER1n_Type * const pTIMER1x = TIMER11;
static const IRQn_Type TIMER1x_IRQ = TIMER11_IRQn;
static const DAC_RELOAD_SIGNAL_Type RELOAD_SIGNAL = DAC_RELOAD_SIGNAL_TIMER11;
#endif

static TIMER1n_CFG_Type TIMER1n_Config;
static DAC_CFG_Type DAC_Config;
static Bool SysTickFlag;
...
{
	_DBG("TIMER11 Occurred\r\n");	
	if (TIMER1n_GetStatus(TIMER11) & TIMER1n_STATUS_MATCH_INT) {
		TIMER1n_ClearStatus(TIMER11, TIMER1n_STATUS_MATCH_INT);
	}
}
...
static void Main_MainLoop(void)
{
	...
	// Initialize and configure timer1n for DAC trigger
	SCU_SetMCCRx(1, TIMER1n_TYPE, CLKSRC_MCLK, 48);		// MCLK=48MHz, Divider=48, 48MHz/48 = 1MHz (TIMER10_CLK)
	SCU_SetTimer1nClk(T1NCLK_MCCR1);					// Setting Timer1n External Clock source

	TIMER1n_Config.StartSync = DISABLE;
	TIMER1n_Config.ClearSync = DISABLE;
	TIMER1n_Config.ExtClock = DISABLE;
	TIMER1n_Config.Mode = TIMER1n_MODE_PERIODIC;
	TIMER1n_Config.MatchInterrupt = ENABLE;
	TIMER1n_Config.CaptureInterrupt = DISABLE;
	TIMER1n_Config.PrescalerData = 1000;				// 1MHz/1000 = 1kHz
	TIMER1n_Config.AData = 1000 - 1;					// 1kHz/(999+1) = 1Hz = 1s
	TIMER1n_Init(pTIMER1x, &TIMER1n_Config);			// Setting Timer1n Configuration

	// Timer10 Interrupt Setting
	NVIC_SetPriority(TIMER1x_IRQ, 3); 
	NVIC_EnableIRQ(TIMER1x_IRQ);
	
	// DAO(PA6) setting
	PCU_ConfigureDirection(PA, 6, PCU_MODE_ALT_FUNC);
	PCU_ConfigureFunction(PA, 6, PCU_ALT_FUNCTION_3);
	PCU_ConfigurePullupdown(PA, 6, PCU_PUPD_DISABLE);

	// DAC setting
	DAC_Config.Stanby = DISABLE;						// DAC normal mode
	DAC_Config.DAC_Out0 = ENABLE;						// Connect with DAO(PA6)
	DAC_Config.OutBuffer = DISABLE;						// Bypass Output Buffer
	DAC_Config.ReloadSignal = RELOAD_SIGNAL;			// Setting Reload Timer
	DAC_Init(DAC, &DAC_Config);							// DAC Initialization

	DAC_SetGain(DAC, DAC_GAIN_0DB);						// No gain
	DAC_SetOffset(DAC, DISABLE, DAC_OFFSET_DIRECTION_MINUS, DAC_OFFSET_VALUE_0);	// Disable DAC Offset

	// Initialize DAC data
	DAC_Data = 0x00;									// Update DAC Initialized Value
	DAC_SetData(DAC, (uint16_t)(DAC_Data << 4));		// Setting DAC value to DAC Data Register 

	// Enable SysTick
	SysTick_Config(SystemCoreClock / 1000); 			// 1ms Systick

	TIMER1n_Start(pTIMER1x);							// Start TIMER10 Operation

	__enable_irq();

	DAC_Start(DAC);										// Start DAC Operation
	
	while (1) {
		DAC_SetData(DAC, DAC_Data << 4);				// Setting DAC Output Value

		_DBG("DAC->DR: "); _DBH32(DAC->DR); _DBG("\r\n");			// Print DAC Output Value
		if (SysTickFlag == TRUE) {
			DAC_Data++;												// Increased DAC Value
			DAC_Data &= 0x0FFFUL;									// Masked DAC value
			SysTickFlag = FALSE;
		}
	}
}
