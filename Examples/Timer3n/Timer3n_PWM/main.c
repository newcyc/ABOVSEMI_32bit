/***************************************************************************//**
* @file     main.c
* @brief    An example of 32-bit timer30 using PWM mode on A31G22x
...
"\t - Using Timer30 PWM mode\r\n"
"************************************************\r\n";

static Bool ADCFlag;
static Bool Timer3nFlag;
...
static TIMER3n_ADC_TRIGGER_CFG_Type TIMER3n_ADC_Trigger_Config;
...
*******************************************************************************/
static void Main_InitializeMPWM(void);
static void Main_InitializeADC(void);
...
/*******************************************************************************
* @brief      Timer30 interrupt handler
* @param      None
* @return     None
*******************************************************************************/
void TIMER30_IRQHandler(void)
{
	if (TIMER3n_GetStatus(TIMER30) & TIMER3n_STATUS_PMATCH_INT) {
		if(Timer3nFlag == TRUE) {
			Timer3nFlag = FALSE;
			GPIO_ClearValue(PC, _BIT(0));
		} else {
			Timer3nFlag = TRUE;
			GPIO_SetValue(PC, _BIT(0));
		}

		TIMER3n_ClearStatus(TIMER30, TIMER3n_STATUS_PMATCH_INT);
	}
}
...
{
	if (ADC_GetStatus(ADC) & ADC_STATUS_SINGLE_INT) {
		if (ADCFlag == TRUE) {
			ADCFlag = FALSE;
			GPIO_ClearValue(PC, _BIT(1));
		} else {
			ADCFlag = TRUE;
			GPIO_SetValue(PC, _BIT(1));
		}

		ADC_ClearStatus(ADC, ADC_STATUS_SINGLE_INT);
	}
}
...
int main(void)
{
	...
	DEBUG_Init(DEBUG_INTERFACE_PERI_UART1);
...
/*******************************************************************************
* @brief      Initialize Timer30(MWPM)
* @param      None
* @return     None
*******************************************************************************/
static void Main_InitializeMPWM(void)
{
	SCU_SetTimer30Clk(T30CLK_PCLK);
	...
	TIMER3n_Config.DelayInsertion = ENABLE;
	...
	TIMER3n_Config.PData = 32000000UL / (PWM_HZ * 2); // 10kHz, 100us(Back to Back)
	TIMER3n_Config.AData = (TIMER3n_Config.PData / 4); // 75.0%
	TIMER3n_Config.BData = (TIMER3n_Config.PData / 3); // 66.7%
	TIMER3n_Config.CData = (TIMER3n_Config.PData / 2); // 50.0%
	TIMER3n_Config.PrescalerData = 0; // 32MHz / (0 + 1) = 32MHz
	TIMER3n_Config.DelayData = 63; // (63 + 1) / 32MHz = 2us
	TIMER3n_Init(TIMER30, &TIMER3n_Config);

	TIMER3n_OutputA_Config.StartLevel = TIMER3n_LEVEL_LOW;
	TIMER3n_OutputA_Config.Output_PWM30Ax = ENABLE;
	TIMER3n_OutputA_Config.Output_PWM30Bx = ENABLE;
	TIMER3n_OutputA_Config.Output_PWM30Cx = ENABLE;
	...
	TIMER3n_OutputB_Config.Output_PWM30Ax = ENABLE;
	TIMER3n_OutputB_Config.Output_PWM30Bx = ENABLE;
	TIMER3n_OutputB_Config.Output_PWM30Cx = ENABLE;
	...
	TIMER3n_ConfigureOutput(TIMER30, TIMER3n_OUTPUT_CTRL_ALL, &TIMER3n_OutputA_Config, &TIMER3n_OutputB_Config);

	TIMER3n_ConfigureInterrupt(TIMER30, TIMER3n_INTERRUPT_PMATCH_INT, ENABLE);

	TIMER3n_ADC_Trigger_Config.Trigger_Bottom = ENABLE;
	TIMER3n_ADC_Trigger_Config.Trigger_PMatch = DISABLE;
	TIMER3n_ADC_Trigger_Config.Trigger_AMatch = DISABLE;
	TIMER3n_ADC_Trigger_Config.Trigger_BMatch = DISABLE;
	TIMER3n_ADC_Trigger_Config.Trigger_CMatch = DISABLE;
	TIMER3n_ConfigureADCTrigger(TIMER30, &TIMER3n_ADC_Trigger_Config);
...
static void Main_InitializeADC(void)
{
	...
	ADC_Config.TriggerSelection = ADC_TRIGGER_SEL_MPWM0;

	ADC_Config.SamplingTime = 1;
	ADC_Config.SequenceCounter = 1;

	ADC_Config.ExternalClock = ENABLE;
	ADC_Config.InternalClockDivider = 1;

	ADC_Init(ADC, &ADC_Config);
	ADC_ConfigureInterrupt(ADC, ADC_INTERRUPT_SINGLE, ENABLE);
	ADC_ConfigureOneSequence(ADC, ADC_SEQUENCE_0, ADC_CHANNEL_0);
...
static void Main_MainLoop(void)
{
	...
	// Set test output pin
	PCU_ConfigureDirection(PC, 0, PCU_MODE_PUSH_PULL);
	PCU_ConfigurePullupdown(PC, 0, PCU_PUPD_DISABLE);	
	GPIO_ClearValue(PC, _BIT(0));
	PCU_ConfigureDirection(PC, 0, PCU_MODE_PUSH_PULL);
	PCU_ConfigurePullupdown(PC, 0, PCU_PUPD_DISABLE);	
	GPIO_ClearValue(PC, _BIT(1));
	...
	// Set PWM pin
	// PWM30AA(PE0)
	PCU_ConfigureDirection(PE, 0, PCU_MODE_ALT_FUNC);
	PCU_ConfigureFunction(PE, 0, PCU_ALT_FUNCTION_1);
	// PWM30AB(PE1)
	PCU_ConfigureDirection(PE, 1, PCU_MODE_ALT_FUNC);
	PCU_ConfigureFunction(PE, 1, PCU_ALT_FUNCTION_1);
	// PWM30BA(PE2)
	PCU_ConfigureDirection(PE, 2, PCU_MODE_ALT_FUNC);
	PCU_ConfigureFunction(PE, 2, PCU_ALT_FUNCTION_1);
	// PWM30BB(PE3)
	PCU_ConfigureDirection(PE, 3, PCU_MODE_ALT_FUNC);
	PCU_ConfigureFunction(PE, 3, PCU_ALT_FUNCTION_1);
	// PWM30CA(PE4)
	PCU_ConfigureDirection(PE, 4, PCU_MODE_ALT_FUNC);
	PCU_ConfigureFunction(PE, 4, PCU_ALT_FUNCTION_1);
	// PWM30CB(PE5)
	PCU_ConfigureDirection(PE, 5, PCU_MODE_ALT_FUNC);
	PCU_ConfigureFunction(PE, 5, PCU_ALT_FUNCTION_1);
