/***************************************************************************//**
* @file     main.c
* @brief    An example of Flash erase/write using self PGM on A31G22x
...
void	tt_flash_iap_pgm(void)
{
	...
	ProgramPage(0xFE00,512, fbuffer);
	GPIO_SetValue(PE,_BIT(0));
	ProgramPage(0x3F000000,512, fbuffer);
	GPIO_ClearValue(PE,_BIT(0));
	ProgramPage(0x3F000200,512, fbuffer);
	GPIO_SetValue(PE,_BIT(0));
	ProgramPage(0x3F000400,512, fbuffer);	
	GPIO_ClearValue(PE,_BIT(0));
	__enable_irq();
}
...
void	tt_flash_iap_erase(void)
{
	...
	EraseSector(0xFF00);
	GPIO_SetValue(PE,_BIT(0));
	EraseSector(0x3F000000);	
	GPIO_ClearValue(PE,_BIT(0));
	EraseSector(0x3F000200);
	GPIO_SetValue(PE,_BIT(0));
	EraseSector(0x3F000400);	
	GPIO_ClearValue(PE,_BIT(0));	
	__enable_irq();
}
