...
void pwmTest(void)
{
	...
	//
	//	PCLK = 74MHz
	// 	PWM Clock = 74MHz / 2 = 37MHz
	// 	one-tick time = 0.027us
	//
	//	period = 1ms
	//	count = 1ms / (1/27) us = 1/37,000
	//
	//----------------------------------------------------------------------------------
	pwm_config.sync_mode = PWM_ASYNC_MODE; 
	pwm_config.invert = PWM_SIG_NO_INVERT;
	pwm_config.clock_select = PWMnCTRL_CKSEL_DIV_BY_2; 
	pwm_config.period = 37000-1;
	pwm_config.compare_val = (37000/2)-1;
