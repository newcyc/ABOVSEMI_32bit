...
*******************************************************************************/
static uint32_t Main_Average8Data(uint32_t *pData);
...
*******************************************************************************/
/*******************************************************************************
* @brief      Average of 8 Data
* @param      pData : Pointer contains 8 data
...
static void Main_MainLoop(void)
{
	...
	while (1) {
		for (i = 0; i < 4; i++) {
			ADC_ConfigureOneSequence(ADC, ADC_SEQUENCE_0, (ADC_CHANNEL_Type)(ADC_CHANNEL_0 + i));
			for (j = 0; j < 8; j++) {
				ADC_Start(ADC);

				while ((ADC_GetStatus(ADC) & ADC_STATUS_SINGLE_INT) == 0);
				ADC_ClearStatus(ADC, ADC_STATUS_SINGLE_INT);

				ADCData[j] = ADC_GetConversionData(ADC, ADC_DATA_SEL_LAST);
			}
		
			Average = Main_Average8Data(ADCData);
			_DBG("AN"); _DBD8(i); _DBG(" = "); _DBH16(Average); _DBG_(".");
		}
