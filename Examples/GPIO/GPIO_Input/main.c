/***************************************************************************//**
* @file     main.c
* @brief    An example of GPIO using input mode on A31G22x
...
*******************************************************************************/
typedef struct {
	PORT_Type *pTarget_PortAddress;
	uint32_t Target_PortNumber;
} PORT_TARGET_Type;
...
* @return     String lenth
*******************************************************************************/
static uint32_t Main_GetString(uint8_t *pBuffer)
{
	uint8_t InputData;
	uint32_t InputCount;
	...
	while (1) {
		...
		if((InputData >= 0x20) && (InputData <= 0x7E)) {
			...
			if (InputCount < BUFFER_LENGTH_MAX) {
				_DBC(InputData);
...
* @param      pTarget : Pointer contains configuration of target port
* @param      pBuffer : Pointer of string buffer
* @param      Length : Length of string buffer
* @return     TRUE : Valid data, FALSE : Invalid data
*******************************************************************************/
static Bool Main_GetTargetPort(PORT_TARGET_Type *pTarget, uint8_t *pBuffer, uint32_t Length)
{
	Bool Result;
	uint8_t Group;
	uint32_t Digit0;
	...
	Result = FALSE;
	if (Length == LENGTH_PORT_NAME) {
		Group = pBuffer[0];
		switch (Group) {
			case 'A' :
				pTarget->pTarget_PortAddress = PA;
				break;
			case 'B' :
				pTarget->pTarget_PortAddress = PB;
				break;
			case 'C' :
				pTarget->pTarget_PortAddress = PC;
				break;
			case 'D' :
				pTarget->pTarget_PortAddress = PD;
				break;
			case 'E' :
				pTarget->pTarget_PortAddress = PE;
				break;
			case 'F' :
				pTarget->pTarget_PortAddress = (PORT_Type *)PF;
				break;
			default :
				Group = 0x00;
				break;
		...
		if (Group) {
			...
			if (((Digit1 == 0) && (Digit0 < 10)) // 0 ~ 9
				|| ((Digit1 == 1) && (Digit0 < 6))) // 10 ~ 15
				{
				pTarget->Target_PortNumber = (Digit1 * 10) + Digit0;
				Result = TRUE;
			}
		}
	}
...
static void Main_MainLoop(void)
{
	...
	PORT_TARGET_Type TargetPort;
	uint8_t StringBuffer[BUFFER_LENGTH_MAX];
	...
	/* Get some inputs */
	_DBG_("\r\nPlease enter Test PORT Name (ex A01)");

	while (1) {
		StringLength = Main_GetString(&(StringBuffer[0]));

		if (StringLength == LENGTH_PORT_NAME) {
			if(Main_GetTargetPort(&TargetPort, &(StringBuffer[0]), LENGTH_PORT_NAME) == TRUE) {
				_DBG_("Start Test");
				break;
			}
		}
	...
	PCU_ConfigureDirection(TargetPort.pTarget_PortAddress, TargetPort.Target_PortNumber, PCU_MODE_INPUT);
	PCU_ConfigurePullupdown(TargetPort.pTarget_PortAddress, TargetPort.Target_PortNumber, PCU_PUPD_DISABLE);
	PCU_ConfigureDebounce(TargetPort.pTarget_PortAddress, TargetPort.Target_PortNumber, ENABLE);
	
	SCU_SetMCCRx(4,PD0_TYPE,CLKSRC_LSI,200);
	SCU_SetMCCRx(4,PD1_TYPE,CLKSRC_LSI,200);
	
	while(1) {
		Value = GPIO_ReadValue(TargetPort.pTarget_PortAddress);
		if (Value & (1 << TargetPort.Target_PortNumber)) {
