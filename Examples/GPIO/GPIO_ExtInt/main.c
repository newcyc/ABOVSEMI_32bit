/***************************************************************************//**
* @file     main.c
* @brief    An example of GPIO using external interrupt mode on A31G22x
...
#define USED_EXT_INTERRUPT_PORT_0			(0)
#define USED_EXT_INTERRUPT_PORT_1			(1)
...
" GPIO PB14,15 as source of external Interrupt\r\n"
"************************************************\r\n";
static const uint8_t MENU_MODE_LEVEL[]=
	"\t - 0: Prohibit external Interrupt\r\n"
	"\t - 1: low level Interrupt\r\n"
	"\t - 2: high level Interrupt\r\n";
static const uint8_t MENU_MODE_EDGE[]=
	"\t - 0: Prohibit external Interrupt\r\n"
...
/*******************************************************************************
* @brief      GPIO A/B interrupt handler
* @param      None
* @return     None
*******************************************************************************/
void GPIOAB_IRQHandler(void)
{
	...
	status = GPIO_GetStatus(PA);
	if (status) {
		if (status & (PCU_ISR_BIT_MASK << (USED_EXT_INTERRUPT_PORT_0 * PCU_ISR_BIT_WIDTH))) {
			for (j = 0; j < 5; j++) {
		...
		if (status & (PCU_ISR_BIT_MASK << (USED_EXT_INTERRUPT_PORT_1 * PCU_ISR_BIT_WIDTH))) {
			for (j = 0; j < 5; j++) {
				GPIO_SetValue(PE, _BIT(2));
				Main_delay();
				GPIO_ClearValue(PE, _BIT(2));
				Main_delay();
			}
		}

		GPIO_ClearStatus(PA, status);
	}
}
...
static void Main_MainLoop(void)
{
	Bool Flag;
	uint8_t Command;
	PCU_INTERRUPT_MODE_Type Mode;
	PCU_INTERRUPT_CTRL_Type Control;

	Main_PrintMenu();

	// Test Pin setting (PE0/PE1)
	...
	PCU_ConfigureDirection(PE, 2, PCU_MODE_PUSH_PULL);
	PCU_ConfigurePullupdown(PE, 2, PCU_PUPD_DISABLE);
	GPIO_ClearValue(PE, _BIT(2));

	// external interrupt test pin(PA00 / PA01)
	PCU_ConfigureDirection(PA, USED_EXT_INTERRUPT_PORT_0, PCU_MODE_INPUT);
	PCU_ConfigurePullupdown(PA, USED_EXT_INTERRUPT_PORT_0, PCU_PUPD_PULL_UP);
	PCU_ConfigureDebounce(PA, USED_EXT_INTERRUPT_PORT_0, ENABLE);

	PCU_ConfigureDirection(PA, USED_EXT_INTERRUPT_PORT_1, PCU_MODE_INPUT);
	PCU_ConfigurePullupdown(PA, USED_EXT_INTERRUPT_PORT_1, PCU_PUPD_PULL_UP);
	PCU_ConfigureDebounce(PA, USED_EXT_INTERRUPT_PORT_1, ENABLE);

	SCU_SetMCCRx(4,PD0_TYPE,CLKSRC_LSI,100);
	
	/* Get some inputs */
	_DBG("\r\nPlease input character 0:LEVEL or 1: EDGE!\r\n");	

	Mode = PCU_INTERRUPT_MODE_DISABLE;
	Flag = FALSE;
	while (Flag == FALSE) {
		Command = _DG;
		switch (Command) {
			case '0' :
				_DBC(Command); _DBG_(" ");
				Mode = PCU_INTERRUPT_MODE_LEVEL;
				Flag = TRUE;
				break;

			case '1': 
				_DBC(Command); _DBG_(" ");
				Mode = PCU_INTERRUPT_MODE_EDGE;
				Flag = TRUE;
				break;
	...
	if (Mode == PCU_INTERRUPT_MODE_LEVEL) {
		_DBG(MENU_MODE_LEVEL);
	} else {
		_DBG(MENU_MODE_EDGE);
	}

	Control = PCU_INTERRUPT_CTRL_PROHIBIT;
	Flag = FALSE;
	while (Flag == FALSE) {
		Command = _DG;
		if (Mode == PCU_INTERRUPT_MODE_LEVEL) {
			switch (Command) {
				...
				case '1' :
					_DBC(Command); _DBG_(" ");
					Control = PCU_INTERRUPT_CTRL_LEVEL_LOW;
					Flag = TRUE;
					break;

				case '2' :
					_DBC(Command); _DBG_(" ");
					Control = PCU_INTERRUPT_CTRL_LEVEL_HIGH;
					Flag = TRUE;
					break;

				default :
					_DBG("\r\n...Please input digit from 0 to 2 only!\r\n");
					break;
			}			
		} else {
			switch (Command) {
				...
				case '1' :
					_DBC(Command); _DBG_(" ");
					Control = PCU_INTERRUPT_CTRL_EDGE_FALLING;
					Flag = TRUE;
					break;

				case '2' :
					_DBC(Command); _DBG_(" ");
					Control = PCU_INTERRUPT_CTRL_EDGE_RISING;
					Flag = TRUE;
					break;

				case '3' :
					_DBC(Command); _DBG_(" ");
					Control = PCU_INTERRUPT_CTRL_EDGE_BOTH;
					Flag = TRUE;
					break;

				default :
					_DBG("\r\n...Please input digit from 0 to 3 only!\r\n");
					break;
			}			
		}
	}

	GPIO_ConfigureInterrupt(PA, USED_EXT_INTERRUPT_PORT_0, Mode, Control);
	GPIO_ConfigureInterrupt(PA, USED_EXT_INTERRUPT_PORT_1, Mode, Control);
