/***************************************************************************//**
* @file     main.c
* @brief    An example of watch timer on A31G22x
...
"************************************************\r\n";
#if defined(USED_INTERRUPT)
static const uint32_t TESTPIN_NUMBER = 0; // PE0
#else
static const uint32_t TESTPIN_NUMBER = 1; // PE1
#endif

static uint32_t TestPin_Output;
...
/*******************************************************************************
* @brief      Watch timer interrupt handler
* @param      None
* @return     None
*******************************************************************************/
void WT_IRQHandler(void)
{
	if (WT_GetInterruptStatus() == SET) {
		if (TestPin_Output == 1) {
			GPIO_ClearValue(PE, _BIT(TESTPIN_NUMBER));
		} else {
			GPIO_SetValue(PE, _BIT(TESTPIN_NUMBER));
		}
		TestPin_Output ^= 1;
		WT_ClearInterruptStatus();
	}
}
...
static void Main_MainLoop(void)
{
	WT_CFG_Type WT_Config;

	Main_PrintMenu();

	// Test Pin setting PE0
	PCU_ConfigureDirection(PE, TESTPIN_NUMBER, PCU_MODE_PUSH_PULL);
	PCU_ConfigurePullupdown(PE, TESTPIN_NUMBER, PCU_PUPD_DISABLE);
	GPIO_ClearValue(PE, _BIT(TESTPIN_NUMBER));

	// select WT clock src: WTCLK_MCCR3, WTCLK_SOSC, WTCLK_WDTRC
#if 0 //WT clock source from MCCR3
	SCU_SetMCCRx(3, WT_TYPE, CLKSRC_HSI, 1);
	SCU_SetWtClk(WTCLK_MCCR3);

	WT_Config.ClockDivider = WT_DIVIDER_2_14_MUL_WTDR; // 48MHz / ((2^14) *(1+WTDR)) = 0.71Hz = 1.398s
	WT_Config.MatchData = 4095; // WTDR Value
#elif 1 //WT clock source from LSE
	PCU_ConfigureDirection((PORT_Type*)PF, 2, PCU_MODE_ALT_FUNC);
	PCU_ConfigureFunction((PORT_Type*)PF, 2, PCU_ALT_FUNCTION_3);
	PCU_ConfigurePullupdown((PORT_Type*)PF, 2, PCU_PUPD_DISABLE);

	PCU_ConfigureDirection((PORT_Type*)PF, 3, PCU_MODE_ALT_FUNC);
	PCU_ConfigureFunction((PORT_Type*)PF, 3, PCU_ALT_FUNCTION_3);
	PCU_ConfigurePullupdown((PORT_Type*)PF, 3, PCU_PUPD_DISABLE);

	SCU_SetLSE(LSE_EN);
	SCU_WaitForLSEStartUp();
	SCU_SetWtClk(WTCLK_LSE);

	WT_Config.ClockDivider = WT_DIVIDER_2_14_MUL_WTDR; // 32768kHz / ((2^14) *(1+WTDR)) = 1000Hz = 0.001s
	WT_Config.MatchData = 1; // WTDR Value
#elif 0 //WT clock source from WDTRC. WDTRC must set LSI clock enable!!
	SCU_SetLSI(LSI_EN);
	SCU_SetWtClk(WTCLK_WDTRC);

	WT_Config.ClockDivider = WT_DIVIDER_2_14_MUL_WTDR; // 31250Hz / ((2^14) *(1+WTDR)) = 0.95Hz = 1.04s
	WT_Config.MatchData = 1; // WTDR Value
#endif
...
* @return     None
*******************************************************************************/
void Main_InitializeClock(void)
{
	uint32_t i;
