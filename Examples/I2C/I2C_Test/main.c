...
void mainloop(void)
{
	...
	for (;;)
	{
		...
		if (InFlag)
		{
			...
			{
				cputs(">>> I2C0 Master Send Test\r\n");
				i2cMaster();				
			}			
			else if (!strncmp(InData, "2", 1))
			{
				cputs(">>> I2C1 Slave Receive Test\r\n");
				i2cSlave();				
			}	
			InFlag = InCount = 0; 
		}
...
int i2cMaster(void)
{
	...
	I2C_CONFIG		test_config; 

	uint8_t			write_buf[8] = {0x12, 0x34, 0x00, 0x78, 0xAB, 0xCD, 0xEF, 0x77};
	...
	//----------------------------------------------------------------------------------
	//	400kHz @ 74MHz
	//				1/400,000 = 2.5us = 1 i2c clock = SCL High + SCL low + (delay)
	// 
	//				SCL HIGH = 1.25us			1.25us / (1/74MHz)us = 92
	//
	//				SCL LOW = 1.25us 			1.25us/ (1/74MHz)us = 92
	//
	//				SDA SETUP/VALID=300ns		300 ns/ (1/74MHz) us = 0.3 x 40 = 12
	//
	//----------------------------------------------------------------------------------	
	cprintf("\r\n");

	test_config.slave_addr = I2C_ADDR; 
	test_config.general_call = I2C_GENERAL_CALL_DISABLE; 

// 400kHz @ PCLK=74MHz
	test_config.scl_low_duration = I2C_SCLL; 
	test_config.scl_high_duration = I2C_SCLH; 
	test_config.sda_hold_duration = I2C_DATAHOLD; 
	test_config.interval = I2C_INTERVAL; 	

	_I2C_Init (0, I2C_MASTER, &test_config); 
	cprintf("nI2C0 Init\r\n"); 
	slave_addr = ((I2C0->SAR) >> 1);
	cprintf("SLAVEADDR = 0x%08X\r\n", slave_addr); 		

	for(i=0; i<5000000; i++);		// delay for sync with slave
	
	//------------------------------------------------------------------
	// write master data --> I2C communication starts ...
	//------------------------------------------------------------------
	I2C_Write_Data (0, I2C_MASTER, write_buf, count, 0); 

	cprintf("SEND: "); 
	for (i=0; i<count; i++)
	{
		cprintf("0x%02X", write_buf[i]);
		if(i < count-1)
			cprintf(", ");
	}
...
int i2cSlave(void)
{
	...
	uint8_t			read_buf[8] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}; 		
	...
	_I2C_Init (1, I2C_SLAVE, &test_config); 

	cprintf("\r\nI2C1 Init \r\n"); 
	slave_addr = ((I2C1->SAR) >> 1);
	cprintf("SLAVEADDR = 0x%08X\r\n", slave_addr); 
	...
	//------------------------------------------------------------------
	i2c_buffer = I2C_Get_BufferAddr(1, I2C_SLAVE, &result); 
	...
	//------------------------------------------------------------------
	// set "u16RxState" into "I2C_RX_STATE_RECEIVE" state, otherwise, 
	// the control exits the while loop prematurely.
	//-----------------------------------------------------------------
	i2c_buffer->u8RxState = I2C_RX_STATE_RECEIVE; 

	for (delay=0; delay<1000; delay++); 
	while (i2c_buffer->u8RxState == I2C_RX_STATE_RECEIVE); 

			
	//------------------------------------------------------------------
	// DISP
	//------------------------------------------------------------------	
	I2C_Read_Data (1, I2C_SLAVE, read_buf, &count); 

	cprintf("\r\nRECEIVE: "); 
	cprintf("read count=%d\r\n", count);
	cprintf("[0] : 0x%02X, ", read_buf[0]); 
	cprintf("[1] : 0x%02X ", read_buf[1]); 
...
void Disp_MainMenu(void)
{
	...
	cprintf("1. I2C0 Master Send Test\r\n");
	cprintf("2. I2C1 Slave Recieve Test\r\n");
	cprintf("========================================\r\n"); 
}
