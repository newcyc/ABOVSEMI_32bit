/***************************************************************************//**
* @file     main.c
* @brief    An example of SPI using USART polling mode on A31G22x
...
void mainloop(void)
{
	...
	while(1){
		...
		while(!(USART_GetStatus(USART10)&0x40)){} // wait 
		tx_ch = USART10->DR;
		USART10->ST = 0x40;
		...
		while(!(USART_GetStatus(USART11)&0x40)){} // wait 
		tx_ch = USART11->DR;
		USART11->ST = 0x40;
		...
		while(!(USART_GetStatus(USART12)&0x40)){} // wait 
		tx_ch = USART12->DR;
		USART12->ST = 0x40;
		...
		while(!(USART_GetStatus(USART13)&0x40)){} // wait 
