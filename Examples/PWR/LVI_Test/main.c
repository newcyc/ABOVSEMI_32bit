/***************************************************************************//**
* @file     main.c
* @brief    An example of LVI(Low Voltage Indicator) on A31G22x
...
#define LVI_TEST_DELAY_TIME			(100) // 100ms
...
static LVI_CFG_Type LVI_Config;
...
*******************************************************************************/
static void Main_CheckLVI(void);
...
static void Main_CheckLVI(void)
{
	if (LVI_GetInterruptStatus() == SET) {
		LVI_ClearInterruptStatus();
...
static void Main_MainLoop(void)
{
	...
	// Configure LVI
	LVI_Config.EnableIndicator = TRUE;
#if defined(USED_INTERRUPT)
	LVI_Config.EnableInterrupt = TRUE;
#else
	LVI_Config.EnableInterrupt = FALSE;
#endif
	LVI_Config.LVI_Voltage = LVI_VOLTAGE_318;
	LVI_Init(&LVI_Config);
	...
	while (1) {
		MilliSec--;
		if ((MilliSec == 0) || (InterruptFlag == TRUE)) {
			MilliSec = LVI_TEST_DELAY_TIME;
			InterruptFlag = FALSE;
			Main_CheckLVI();
		}
