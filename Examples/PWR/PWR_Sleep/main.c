/***************************************************************************//**
* @file     main.c
* @brief    An example of sleep on A31G22x
...
#define SLEEP_TEST_DELAY_LOOP			(10)
#define SLEEP_TEST_DELAY_TIME			(1000) // 1000ms
...
*******************************************************************************/
static void Main_TestSleep(void);
...
static void Main_TestSleep(void)
{
	...
	_DBG_("Press USER_SW(PF6) to Enter Sleep Mode !"); _DBG_(" ");
	while(GPIO_ReadValue((PORT_Type *)PF) & PORT_INDR_INDR6_Msk);

	_DBG_("Enter Deep Sleep... ");
	_DBG_("Once again press USER_SW(PF6) to Exit Sleep Mode !"); _DBG_(" ");
	for (i = 0; i < 1000; i++) {
		__NOP();
	}
	...
	_DBG_("Waked up from Sleep !"); _DBG_(" ");
...
static void Main_MainLoop(void)
{
	...
	Loop = SLEEP_TEST_DELAY_LOOP;
	while (1) {
		Loop++;
		if (Loop > SLEEP_TEST_DELAY_LOOP) {
			Loop = 0;
			Main_TestSleep();
		}

		MilliSec = SLEEP_TEST_DELAY_TIME;
		while (MilliSec);
