/***************************************************************************//**
* @file     main.c
* @brief    An example of watchdog timer(reset) on A31G22x
...
#define TEST_CASE_0				(0) // WDTDR(0.5s) < WDTWDR(1s), Reload after 0.9s ==> Reset (underflow)
#define TEST_CASE_1				(1) // WDTDR(0.5s) < WDTWDR(1s), Reload after 0.4s ==> No reset
#define TEST_CASE_2				(2) // WDTDR(1s) == WDTWDR(1s), Reload after 0.9s ==> No reset
#define TEST_CASE_3				(3) // WDTDR(1s) == WDTWDR(1s), Reload after 1.2s ==> Reset (underflow)
#define TEST_CASE_4				(4) // WDTDR(1s) > WDTWDR(0.5s), Reload after 0.2s ==> Reset (window)
#define TEST_CASE_5				(5) // WDTDR(1s) > WDTWDR(0.5s), Reload after 0.8s ==> No reset

#define USED_TEST_CASE			(TEST_CASE_0)

#define CLOCK_DIVIDER_TYPE			(WDT_DIVIDER_4)
#if (CLOCK_DIVIDER_TYPE == WDT_DIVIDER_4)
#define CLOCK_DIVIDER_VALUE			(4)
#elif (CLOCK_DIVIDER_TYPE == WDT_DIVIDER_16)
#define CLOCK_DIVIDER_VALUE			(16)
#elif (CLOCK_DIVIDER_TYPE == WDT_DIVIDER_64)
#define CLOCK_DIVIDER_VALUE			(64)
#elif (CLOCK_DIVIDER_TYPE == WDT_DIVIDER_256)
#define CLOCK_DIVIDER_VALUE			(256)
#else
 #error "Out of divider range"
#endif

#define CLOCK_MCLK_HSI			(32000000) // 32MHz
#define CLOCK_WDTRC				(31250) // 31.25kHz

#define CLOCK_WDT				(CLOCK_WDTRC)
#define COUNT_1_SEC				(CLOCK_WDT / CLOCK_DIVIDER_VALUE)
...
"\t - Using Watchdog timer reset mode\r\n"
"************************************************\r\n";
#if (USED_TEST_CASE == TEST_CASE_0) // Reset (underflow)
static const uint32_t WDT_DATA			= (COUNT_1_SEC / 2); // 0.5s
static const uint32_t WDT_WINDOW		= (COUNT_1_SEC); // 1s
static const uint32_t SYSTICK_WAIT_TIME	= (900); // 0.9s
#elif (USED_TEST_CASE == TEST_CASE_1) // No reset
static const uint32_t WDT_DATA			= (COUNT_1_SEC / 2); // 0.5s
static const uint32_t WDT_WINDOW		= (COUNT_1_SEC); // 1s
static const uint32_t SYSTICK_WAIT_TIME 	= (400); // 0.4s
#elif (USED_TEST_CASE == TEST_CASE_2) // No reset
static const uint32_t WDT_DATA			= (COUNT_1_SEC); // 1s
static const uint32_t WDT_WINDOW		= (COUNT_1_SEC); // 1s
static const uint32_t SYSTICK_WAIT_TIME	= (900); // 0.9s
#elif (USED_TEST_CASE == TEST_CASE_3) // Reset (underflow)
static const uint32_t WDT_DATA			= (COUNT_1_SEC); // 1s
static const uint32_t WDT_WINDOW		= (COUNT_1_SEC); // 1s
static const uint32_t SYSTICK_WAIT_TIME	= (1200); // 1.2s
#elif (USED_TEST_CASE == TEST_CASE_4) // Reset (window)
static const uint32_t WDT_DATA			= (COUNT_1_SEC); // 1s
static const uint32_t WDT_WINDOW		= (COUNT_1_SEC / 2); // 0.5s
static const uint32_t SYSTICK_WAIT_TIME	= (200); // 0.2s
#elif (USED_TEST_CASE == TEST_CASE_5) // No Reset
static const uint32_t WDT_DATA			= (COUNT_1_SEC); // 1s
static const uint32_t WDT_WINDOW		= (COUNT_1_SEC / 2); // 0.5s
static const uint32_t SYSTICK_WAIT_TIME	= (800); // 0.8s
#else
 #error "Out of test case range"
#endif
...
static void Main_MainLoop(void)
{
	...
	// select WDT clock
#if (CLOCK_WDT == CLOCK_MCLK_HSI)
	SCU_SetMCCRx(3, WDT_TYPE, CLKSRC_MCLK, 1);
	SCU_SetWdtClk(WDTCLK_MCCR3);
#elif (CLOCK_WDT == CLOCK_WDTRC)
	SCU_SetLSI(LSI_EN);
	SCU_SetWdtClk(WDTCLK_WDTRC);
#else
 #error "Out of clock range"
#endif

	SCU_SetResetSrc(RST_WDTRST, ENABLE);
	SCU_ClearResetStatus(0xff); // clear all reset status 

	WDT_Config.Reset = ENABLE;
	WDT_Config.ClockDivider = CLOCK_DIVIDER_TYPE;
	WDT_Config.Data = WDT_DATA;
	WDT_Config.WindowData = WDT_WINDOW;

	WDT_Init(&WDT_Config);
	WDT_ConfigureInterrupt(ENABLE, ENABLE);
	...
	while (1) {
		MilliSec = SYSTICK_WAIT_TIME;
		while (MilliSec);
		WDT_ReloadTimeCounter();
	}
}
