...
"Invaild";
static uint8_t TestString[MAX_TEST_STRING] = {
	'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J'
};
...
*******************************************************************************/
static void Main_InitializeDMAC(void);
...
/*******************************************************************************
* @brief      Comparator&CRC interrupt handler
* @param      None
* @return     None
*******************************************************************************/
void CMP_CRC_IRQHandler(void)
{
	...
	Status = CRC_GetInterruptStatus();

	if (Status == SET) {
		DMA_Done = TRUE;
		CRC_ClearInterruptStatus();
	}
}
...
/*******************************************************************************
* @brief      Initialize DMAC(Ch0)
* @param      None
* @return     None
*******************************************************************************/
static void Main_InitializeDMAC(void)
{
	DMAC_CFG_Type Config;

	Config.Peripheral = DMAC_PERIPHERAL_CRC;
	Config.Width = DMAC_WIDTH_BYTE;
	Config.Direction = DMAC_DIRECTION_TX;
	Config.PeripheralAddress = (uint32_t)(&(CRC->IDR));
	DMAC_Init(DMAC0, &Config);
}
...
static void Main_MainLoop(void)
{
	...
	CRC_Config.DMADoneInterrupt = ENABLE;
	...
	NVIC_SetPriority(CMP_CRC_IRQn, 3);
	...
	while (1) {
		...
		if (ValidCommand == TRUE) {
			...
			if (CRC_Config.Polynomial == CRC_POLY_CRC7) {
				Result = Main_CalculateByBit(TestString, MAX_TEST_STRING);
			} else {
				Main_GenerateCRCTable();
				Result = Main_CalculateByTable(TestString, MAX_TEST_STRING);
			}
			...
			DMA_Done = FALSE;
			DMAC_SetTransferCount(DMAC0, MAX_TEST_STRING);
			DMAC_SetMemoryLowAddress(DMAC0, (uint16_t)(((uint32_t)&(TestString[0])) & 0x0000FFFF));
			DMAC_Start(DMAC0);
