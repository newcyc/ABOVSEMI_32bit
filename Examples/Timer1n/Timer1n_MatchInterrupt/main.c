/***************************************************************************//**
* @file     main.c
* @brief    An example of 16-bit timer1n using match(interrupt) mode on A31G22x
...
#define TIMER1n_CLOCK				(USED_CLOCK_MCCR)
...
* Private Typedef
*******************************************************************************/
// Timer1n match demo configuration structure definition
typedef struct {
	// base address
...
"\t - Using timer1n match interrupt mode\r\n"
"************************************************\r\n";

static const TIMER1n_MATCH_TYPE TIMER1n_MATCH_CFG[TIMER1n_IP_INDEX_MAX] = {
	{TIMER10, TIMER10_IRQn,              PE,  6, PCU_ALT_FUNCTION_1,              PC, 11, PCU_ALT_FUNCTION_1},
	{TIMER11, TIMER11_IRQn,              PE,  7, PCU_ALT_FUNCTION_1,              PC, 12, PCU_ALT_FUNCTION_1},
	{TIMER12, TIMER12_IRQn,              PA,  5, PCU_ALT_FUNCTION_1,              PA,  2, PCU_ALT_FUNCTION_1},
	{TIMER13, TIMER13_IRQn, (PORT_Type *)PF, 10, PCU_ALT_FUNCTION_1, (PORT_Type *)PF,  8, PCU_ALT_FUNCTION_1},
	{TIMER14, TIMER14_IRQn, (PORT_Type *)PF, 11, PCU_ALT_FUNCTION_1, (PORT_Type *)PF,  9, PCU_ALT_FUNCTION_1},
	{TIMER15, TIMER15_IRQn,              PB,  8, PCU_ALT_FUNCTION_1,              PB, 10, PCU_ALT_FUNCTION_2},
	{TIMER16, TIMER16_IRQn,              PB,  9, PCU_ALT_FUNCTION_1,              PB, 11, PCU_ALT_FUNCTION_2}
};

static Bool Timer1nFlag;
static uint32_t Timer1nCount;
...
static void Main_Timer1nIRQHandler(TIMER1n_Type *pTIMER1x)
{
	if (TIMER1n_GetStatus(pTIMER1x) & TIMER1n_STATUS_MATCH_INT) {
		Timer1nCount++;
		if (Timer1nCount >= 200) {
			Timer1nCount = 0;
		...
		TIMER1n_ClearStatus(pTIMER1x, TIMER1n_STATUS_MATCH_INT);
	}
}
...
static void Main_MainLoop(void)
{
	const TIMER1n_MATCH_TYPE * pConfig;

	Timer1nFlag = FALSE;
	Timer1nCount = 0;

	pConfig = &(TIMER1n_MATCH_CFG[TARGET_TIMER]);
	...
	// Timer1n and clock source setting (MCCR1 / PCLK / ECn)
#if (TIMER1n_CLOCK == USED_CLOCK_MCCR)
	SCU_SetMCCRx(1, TIMER10_TYPE, CLKSRC_MCLK, 2);
	SCU_SetTimer1nClk(T1NCLK_MCCR1); 

	TIMER1n_Config.ExtClock = DISABLE; // MCCR 16MHz
	TIMER1n_Config.PrescalerData = 16 - 1; // 16MHz / 16 = 1MHz -> 1us
#elif (TIMER1n_CLOCK == USED_CLOCK_PCLK)
...
#elif (TIMER1n_CLOCK == USED_CLOCK_EC)
	...
	TIMER1n_Config.ExtClock = ENABLE; // EC
	TIMER1n_Config.ExtClockEdge = TIMER1n_EXT_CLOCK_EDGE_FALLING;
	TIMER1n_Config.PrescalerData = 0; // Bypass
#endif
	...
	TIMER1n_Config.AData = 1000 - 1; // 1ms
