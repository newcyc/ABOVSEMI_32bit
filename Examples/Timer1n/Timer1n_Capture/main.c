/***************************************************************************//**
* @file     main.c
* @brief    An example of 16-bit timer1n using capture mode on A31G22x
...
#define TARGET_TIMER				(TIMER1n_IP_INDEX_TIMER10) // TIMER1n_INDEX_TIMER10 ~ TIMER1n_INDEX_TIMER16


/*******************************************************************************
* Private Typedef
*******************************************************************************/
// Timer1n capture demo configuration structure definition
typedef struct {
	// base address
	...
	// Capture Port
	PORT_Type *pCapturePort_Group;
	uint32_t CapturePort_Number;
	PCU_ALT_FUNCTION_Type CapturePort_AltFunc;
} TIMER1n_CAPTURE_Type;
...
"\t - Using timer1n Capture mode\r\n"
"************************************************\r\n";

static const TIMER1n_CAPTURE_Type TIMER1n_CAPTURE_CFG[TIMER1n_IP_INDEX_MAX] = {
	{TIMER10, TIMER10_IRQn,              PE,  6, PCU_ALT_FUNCTION_2},
	{TIMER11, TIMER11_IRQn,              PE,  7, PCU_ALT_FUNCTION_2},
	{TIMER12, TIMER12_IRQn,              PA,  5, PCU_ALT_FUNCTION_2},
	{TIMER13, TIMER13_IRQn, (PORT_Type *)PF, 10, PCU_ALT_FUNCTION_2},
	{TIMER14, TIMER14_IRQn, (PORT_Type *)PF, 11, PCU_ALT_FUNCTION_2},
	{TIMER15, TIMER15_IRQn,              PB,  8, PCU_ALT_FUNCTION_2},
	{TIMER16, TIMER16_IRQn,              PB,  9, PCU_ALT_FUNCTION_2}
};

static Bool Timer1nFlag;
...
*******************************************************************************/
static void Main_Timer1nIRQHandler(TIMER1n_Type *pTIMER1x);
...
{
	Main_Timer1nIRQHandler(TIMER10);
}

/*******************************************************************************
* @brief      Timer11 interrupt handler
* @param      None
* @return     None
*******************************************************************************/
void TIMER11_IRQHandler(void)
{
	Main_Timer1nIRQHandler(TIMER11);
}
...
{
	Main_Timer1nIRQHandler(TIMER12);
}

/*******************************************************************************
* @brief      Timer13 interrupt handler
* @param      None
* @return     None
*******************************************************************************/
void TIMER13_IRQHandler(void)
{
	Main_Timer1nIRQHandler(TIMER13);
}

/*******************************************************************************
* @brief      Timer14 interrupt handler
* @param      None
* @return     None
*******************************************************************************/
void TIMER14_IRQHandler(void)
{
	Main_Timer1nIRQHandler(TIMER14);
}

/*******************************************************************************
* @brief      Timer15 interrupt handler
* @param      None
* @return     None
*******************************************************************************/
void TIMER15_IRQHandler(void)
{
	Main_Timer1nIRQHandler(TIMER15);
}

/*******************************************************************************
* @brief      Timer16 interrupt handler
* @param      None
* @return     None
*******************************************************************************/
void TIMER16_IRQHandler(void)
{
	Main_Timer1nIRQHandler(TIMER16);
}
...
/*******************************************************************************
* @brief      Timer1n Interrupt Handler
* @param      None
* @return     None
*******************************************************************************/
static void Main_Timer1nIRQHandler(TIMER1n_Type *pTIMER1x)
{
	if (TIMER1n_GetStatus(pTIMER1x) & TIMER1n_STATUS_CAPTURE_INT) {
		if (Timer1nFlag == FALSE) {
			Timer1nFlag = TRUE;
		}

		TIMER1n_ClearStatus(pTIMER1x, TIMER1n_STATUS_CAPTURE_INT);
	}
}
...
static void Main_MainLoop(void)
{
	volatile uint32_t i;
	uint32_t Count;
	const TIMER1n_CAPTURE_Type * pConfig;

	pConfig = &(TIMER1n_CAPTURE_CFG[TARGET_TIMER]);
	...
	SCU_SetMCCRx(4, PD0_TYPE, CLKSRC_LSI, 5); // 1 clock = 10us
	SCU_SetMCCRx(4, PD1_TYPE, CLKSRC_LSI, 5); // 1 clock = 10us

	PCU_ConfigureDirection(pConfig->pCapturePort_Group, pConfig->CapturePort_Number, PCU_MODE_ALT_FUNC);
	PCU_ConfigureFunction(pConfig->pCapturePort_Group, pConfig->CapturePort_Number, pConfig->CapturePort_AltFunc);
	PCU_ConfigureDebounce(pConfig->pCapturePort_Group, pConfig->CapturePort_Number, ENABLE);
	...
	TIMER1n_Config.Mode = TIMER1n_MODE_CAPTURE;
	...
	TIMER1n_Config.PrescalerData = 32 - 1; // 32MHz / 32 = 1MHz -> 1us

	TIMER1n_Init(pConfig->pTIMER1x, &TIMER1n_Config);
	...
	Timer1nFlag = FALSE;
	TIMER1n_ClearCounter(pConfig->pTIMER1x);
	TIMER1n_Start(pConfig->pTIMER1x);
	...
	while (1) {
		if (Timer1nFlag == TRUE) {
			_DBG_("capture value!");
			Count = TIMER1n_GetCaptureData(pConfig->pTIMER1x); 
			TIMER1n_ClearCounter(pConfig->pTIMER1x);
			_DBG("Capture CNT = "); _DBD32(Count); _DBG_(" ");
			Timer1nFlag = FALSE;
		}
	}
}
