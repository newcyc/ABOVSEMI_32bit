...
extern uint32_t g_LVDCON_val;

extern void LVDFAIL_Handler(void);			// Refers IRQ_Hancler.c source file
...
static void GetLviStatus(void);
void LowVoltageInterruptTest(void);
...
{
	cprintf("\r\n========================================\r\n");
	cprintf("Low-Voltage Interrupt Demo\r\n");  
	cprintf("\t - MCU : A33G52x\r\n");
...
static void GetLviStatus(void)
{
	...
	if(Reg32 & LVDCON_LVDIEN)			cprintf("Enable Low-Voltage Interrupt\r\n");
	else											cprintf("Disable Low-Voltage Interrupt\r\n");
	
	cprintf("LVI Level : ");
	Reg32 = PMU->LVDCON& LVDCON_LVDIL_MASK;
	
	switch(Reg32)
	{	
		case LVDCON_LVDIL_2_60V:		cprintf("2.60V\r\n");		break;
		case LVDCON_LVDIL_2_80V:		cprintf("2.80V\r\n");		break;
		case LVDCON_LVDIL_3_00V:		cprintf("3.00V\r\n");		break;
		case LVDCON_LVDIL_3_30V:		cprintf("3.30V\r\n");		break;
		case LVDCON_LVDIL_3_75V:		cprintf("3.75V\r\n");		break;
		case LVDCON_LVDIL_4_00V:		cprintf("4.00V\r\n");		break;
		case LVDCON_LVDIL_4_25V:		cprintf("4.25V\r\n");		break;
		case LVDCON_LVDIL_4_50V:		cprintf("4.50V\r\n");		break;
		default: 																		break;
	}
	cputs("\r\n");
}

extern uint32_t LvICnt;
void LowVoltageInterruptTest(void)
{
	GetLviStatus();						// Print the Low-voltage interrupt status before setting

	// Disable Interrupt and Interrupt Flag 
	NVIC_ClearPendingIRQ(LVDDETECT_IRQn);	// Clear NVIC Interrupt				
	NVIC_DisableIRQ(LVDDETECT_IRQn);				// DISABLE NVIC Interrupt
	__disable_irq();	

	PMU->LVDCON |= LVDCON_LVDIF;			// Clear LVI Flag

	// Configuration of LVD Interrupt
//	PMU->LVDCON = (LVDCON_LVDEN | LVDCON_LVDIEN | LVDCON_LVDIL_2_60V);			// LVD Interrupt @ 2.60V	
//	PMU->LVDCON = (LVDCON_LVDEN | LVDCON_LVDIEN | LVDCON_LVDIL_2_80V);			// LVD Interrupt @ 2.80V	
//	PMU->LVDCON = (LVDCON_LVDEN | LVDCON_LVDIEN | LVDCON_LVDIL_3_00V);			// LVD Interrupt @ 3.00V	
//	PMU->LVDCON = (LVDCON_LVDEN | LVDCON_LVDIEN | LVDCON_LVDIL_3_30V);			// LVD Interrupt @ 3.30V	
//	PMU->LVDCON = (LVDCON_LVDEN | LVDCON_LVDIEN | LVDCON_LVDIL_3_75V);			// LVD Interrupt @ 3.75V	
	PMU->LVDCON = (LVDCON_LVDEN | LVDCON_LVDIEN | LVDCON_LVDIL_4_00V);			// LVD Interrupt @ 4.00V	
//	PMU->LVDCON = (LVDCON_LVDEN | LVDCON_LVDIEN | LVDCON_LVDIL_4_25V);			// LVD Interrupt @ 4.25V
//	PMU->LVDCON = (LVDCON_LVDEN | LVDCON_LVDIEN | LVDCON_LVDIL_4_50V);			// LVD Interrupt @ 4.50V	

	// Enable Interrupt
	NVIC_ClearPendingIRQ(LVDDETECT_IRQn);
	NVIC_SetPriority(LVDDETECT_IRQn, 3);			// Set LVI Interrupt
	NVIC_EnableIRQ(LVDDETECT_IRQn);				// Enable LVI Interrupt	
	__enable_irq();	
	
	GetLviStatus();						// Print  LVI status
	
	// Try Adjusting the input voltage level of the demo board in this state.	
	while(1)
	{
		if(LvICnt > 0) {
			cprintf("!! Occurred LVD Interrupt !! \r\n");
			LvICnt  = 0;
		}
	}
